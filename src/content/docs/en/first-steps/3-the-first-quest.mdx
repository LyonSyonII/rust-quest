...
`Welcome Adventurer.
Your first mission is... To help the local grocer sort some vegetables!
...
<QuestionCard question="Read your first mission!">
    <CodeBlock client:only code="read_mission()" setup={`fn read_mission() { println!("${mission}") } __VALUE__`} editable={false} showLineNumbers={false} />
</QuestionCard>
