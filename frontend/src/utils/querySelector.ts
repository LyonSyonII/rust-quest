export function $<S extends string, T extends Element = Element>(
  selector: S,
  scope: ParentNode = document,
): ParseSelector<S, T> {
  const s = scope.querySelector<S>(selector) as ParseSelector<S, T> | null;
  if (s === null) {
    throw `selector ${selector} did not match any node`;
  }
  return s;
}

export function querySelectorAll<S extends string, T extends Element = Element>(
  selector: S,
  scope: ParentNode = document,
): NodeListOf<ParseSelector<S, T>> {
  return scope.querySelectorAll<S>(selector) as NodeListOf<ParseSelector<S, T>>;
}
