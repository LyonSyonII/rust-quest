...
function validator(value: string): string | undefined {
  const regex = createRegExp(
    start, _,
    line, _,
    "if", _, exactly("?").or(word, "()", maybe(";")).as("first"), _, "{", _, line, _, line, _, line.as("extra1"), _, "}", _,
    "if", _, exactly("?").or(word, "()", maybe(";")).as("second"), _, "{", _, line, _, line, _, line.as("extra2"), _, "}", _,
    end
  );
  const matches = value.match(regex);
  if (!matches) return codeMess;

  const { first, second, extra1, extra2 } = matches.groups;
  ...
  return value.includes("?") && replace
    || (extra1 || extra2) && "You don't need more than two lines in each condition!"
