...
const validator: Validator = (value, test) => {
  ...
  return (height.includes("?") || height.length == 0) && "[height] Fill in your height!"
      || /^\d+,\d+$/.test(height) && "[height] Close! But in most programming languages a dot '.' is used for separating decimal numbers."
