...
import { codeMessQuestion, mc, mo, replace, type Validator } from "./CodeQuestion";
...
const validator: Validator = (value: string) => {
  ...
  const regex = createRegExp(
    ...
    "if",
    _,
    exactly("?").or(fn, maybe(";")).as("first"),
    _,
    "{",
    _,
    instructions.as("inst1"),
    "}",
    _,
    "if",
    _,
    exactly("?").or(fn, maybe(";")).as("second"),
    _,
    "{",
    _,
    instructions.as("inst2"),
    "}",
    _,
    end,
  );
