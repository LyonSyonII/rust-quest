...
const validator: Validator = (value, test) => {
  ...
  return (
    (test(/let name = "";/) && "[name] Fill in your name!") ||
    (!test(/let name = ".+";/) &&
      "[name] Something is wrong with your name, click the reset button if you changed it by mistake!") ||
    (test(/let surname = (\?|"");/) && "[surname] Fill in your surname!") ||
    (!surname.includes('"') &&
      "[surname] Something is wrong with your surname.\nLook closely to how 'name' is written!") ||
