...
If the temperature is too low, the target will be raised to the maximum, and viceversa, ensuring the actual temperature is always between the minimum and the maximum.
...
| `target`        | Variable that represents the **target temperature**            |
| `temp`        | Variable that represents the **current temperature**            |
</QuestionCard>
...
<CodeQuestion
  ...
  id="6-1"
  code={`
// if above MAX, set target to MIN
if temp > MAX_TEMP {
  target = MAX_TEMP;
} 
// if below MIN, set target to MAX
