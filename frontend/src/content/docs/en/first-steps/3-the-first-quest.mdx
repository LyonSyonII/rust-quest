...
<Checkpoint id="3-1" scroll={false} >
  ...
  <Comic src="box-empty-opt" alt="empty box" eager />
  ...
  <Hint>
    The structure of the code is `let <label> = <contents>`, think of the label as being the literal contents of the crate.
...
<Checkpoint id="3-2">
:::tip[Exercise]
Try changing the value of each _label_ and running the code again.  
You'll see that the result changes!
:::
...
<Comic src="box-apples-18-opt" alt="apple box" />

:::note[Variables]
You've just created a variable!  

Variables are used to give a name to certain values.  
In this case, `apples` means "18 units of a fruit called apple".  
:::
...
Sure enough, the panel glows red, and says: <Highlight bg={colors.red}>Anti-theft protection enabled, if you want to change the contents of this box **add the `mut` directive**</Highlight>
...
`let mut apples = 16` means "make these apples changeable and set them to 16", where "mut" is "mutable".
