...
This class requires a small test, we're going to prove you have the minimum problem-solving skills required.  
...
<Checkpoint id="2-3">
  :::note[Seems like normal math?]
  Exactly! If you've worked with Excel before, it should feel very familiar.  
  This syntax is shared between almost all programming languages.

  Have you noticed the double equal sign `==`?  
  As you can imagine, it means "the left expression is equal to the right expression".  

  This symbol is also used in many languages, although some give it slightly different meanings.

  Why a double sign you say?  
