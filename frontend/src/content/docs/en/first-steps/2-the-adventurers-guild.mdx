...
import PortraitCard from "@components/PortraitCard/PortraitCard.astro";
...
export function FreeQuestion({ question, code = "", correct, id }) {
  const setup = `
        if __VALUE__ { println!("${correct}SUCCESS\\n"); } else { println!("${wrong_answer}"); }`.replaceAll("\n", "");
  const validator = String(
    /** @type { (value: string) => (string | undefined) } */
    (value) => 
      value.includes("?") && `REPLACE` ||
...
{ /** TODO! Circumvent bug: If not defined in a function, all Svelte components will not load */}
export function Q({}) {
  return (
    <CodeQuestion
      question=""
      ...
      validator={`(value) => value.includes("?") && "Hint: Just replace ? with your name!" || undefined`}
    />
  )
}
