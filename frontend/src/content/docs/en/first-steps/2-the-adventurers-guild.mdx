...
export function SimpleQuestion({
  ...
  const validator = String(
    ...
    (value) => 
      !value.startsWith("NO_DUMMY") && "Seems like you've messed up the code, click the \"Reset\" button to return it back to its original state.\nRemember to only modify the ? symbol."
      || value.includes("?") && "Replace ? with your answer."
      || undefined
  ).replace("NO_DUMMY", no_dummy);
  
  return (
    <CodeQuestion question={question} code={code} setup={setup} validator={validator} showLineNumbers={false} />
  );
}
/** @param {{question: string, code: string, correct: string}} */
...
export const validator = String(
  /** @type { (value: string) => (string | undefined) } */
  (value) => value.includes("?") && "Hint: Just replace ? with your name!" || undefined
);
export const pata = Symbol();
